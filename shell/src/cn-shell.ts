...
abstract class CNShell {
  ...
  getConfigStr(userParams: ConfigParams): string {
    userParams.type = ConfigTypes.String;

    return <string>this.getConfig(userParams);
  }

  getConfigBool(userParams: ConfigParams): boolean {
    userParams.type = ConfigTypes.Boolean;

    return <boolean>this.getConfig(userParams);
  }

  getConfigNum(userParams: ConfigParams): number {
    userParams.type = ConfigTypes.Number;

    return <number>this.getConfig(userParams);
  }

  getConfig(userParams: ConfigParams): string | number | boolean {
    // Setup the defaults
    let params = {
    ...
    // Check the CLI first since this has higher precedence then the env vars
    let value = this._minimist[params.config];
    ...
    if (value !== undefined) {
      if (
        ...
        // NOTE: Minimist does conversions so no need to do it here
        return value;
      }
    }

    // OK, not in the CLI so lets try the env vars
    let evar = `CNA_${params.config.toUpperCase()}`;
    ...
    if (value === undefined) {
      ...
      if (params.required) {
        throw Error(
          `Config parameter (${evar}) not set on the CLI or as an env var!`,
        );
      }
    ...
    return this.convertConfig(value, params.type);
  }
